...
public:
	...
	void createCopy(BSTreeInt& tree);
	BSTNodeInt* findNodeWithLargestRightSubtree();
	int diveRight(BSTNodeInt*);
	int deleteAllLeftLeafes(BSTNodeInt* node);
	bool isLeaf(BSTNodeInt* node);
	BSTNodeInt* differenceInNumber();
	bool isSplit(BSTNodeInt* node);
	int dive(BSTNodeInt*);
};
